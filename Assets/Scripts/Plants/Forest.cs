...
public class Forest : MonoBehaviour
{
    ...
    ProceduralTree RandomTree(Vector3 pos, int iterations) {
        ...
        float branchLengthDefault = SampleBellCurveInRange(2f, 4f);

        float branchDefaultWidth = SampleBellCurveInRange(0.5f, 2f);

        float taperWidth = SampleBellCurveInRange(0.1f, 0.5f);

        float taperHeight = SampleBellCurveInRange(0.1f, 0.5f);
    ...
    {
        return ((RandomGaussian() * (max - min)) + min);
    }

    float RandomGaussian()
    {
        return ((float)prng.NextDouble() + (float)prng.NextDouble() + (float)prng.NextDouble())/3f;
    }
